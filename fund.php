...
// Total spend
$db->query('SELECT SUM(CASE WHEN `price2016` > `price2015` THEN `price2016` ELSE `price2015` END) AS `price` FROM `journals` WHERE `subAccount` = :fund AND ((`price2016` IS NOT NULL AND `price2016` > 0) OR (`price2015` IS NOT NULL AND `price2015` > 0)) GROUP BY `subAccount`');
$db->bind('fund', $fund);
$totalSpend = $db->single();
